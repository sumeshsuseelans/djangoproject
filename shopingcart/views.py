...
import random
from .forms import OrderNumberForm
...
class CheckoutView(generic.ListView):
	...
	def get_queryset(self):
		orderNo = random.randint(100000,900000)
		OrderNumber.objects.all().delete()
		OrderNumber.objects.create(orderNo=orderNo)
...
class PlaceOrderView(generic.TemplateView):
	...
	model=Cart
	context_object_name = 'checkout_li'
	...
	def get(self,request):
		...
		q1=Cart.objects.filter(email=email).count()
		request.session['cart_items']=q1
		order_no = OrderNumber.objects.all
		ship_obj = ShippingAddress.objects.all
		return render(request,self.template_name,{'order' : order_no})
...
def ShippingView(request):
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			form.save()
			shipping_objects = ShippingAddress.objects.all
			return render(request, "shopingcart/shipping.html", {'all_shipping' : shipping_objects})
		    

	return render(request, "shopingcart/shipping.html", {})    
